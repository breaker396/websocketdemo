namespace CameraStream.Hubs
{
    ...
    public class ConnectionHub : Hub<IConnectionHub>
    {
            ...
            while (await stream.WaitToReadAsync())
            {
                while (stream.TryRead(out var item))
                {
                    if (!string.IsNullOrEmpty(item))
                    {
                        ...
                        await Clients.All.ReceiveData(callingUser, dataStream[1]);
                    }
        ...
        public ChannelReader<string> DownloadStream(int delay)
        {
            var channel = Channel.CreateUnbounded<string>();
            _ = WriteItemsAsync(channel.Writer, DateTime.Now.Millisecond.ToString(), delay);
